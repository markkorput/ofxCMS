...
// ObjectCollectionBase is a "core-essential" base class for the main Collection class.
// ObjectCollectionBase only implement the bare basics of creating, adding, update and removing items.
...
namespace ofxCMS {
    ...
    class ObjectCollectionBase{
        ...
        public: // methods
            ...
            ObjectCollectionBase() : vectorLockCount(0){}
            ~ObjectCollectionBase(){ destroy(); }
...
template <class ObjectType>
void ofxCMS::ObjectCollectionBase<ObjectType>::destroy(){
    if(isLocked()){
        ofLogWarning() << "ofxCMS::ObjectCollectionBase destroyed while locked";
    }
...
template <class ObjectType>
shared_ptr<ObjectType> ofxCMS::ObjectCollectionBase<ObjectType>::create(){
...
template <class ObjectType>
void ofxCMS::ObjectCollectionBase<ObjectType>::add(shared_ptr<ObjectType> instanceRef, bool notify){
...
template <class ObjectType>
shared_ptr<ObjectType> ofxCMS::ObjectCollectionBase<ObjectType>::previous(shared_ptr<ObjectType> instanceRef, bool wrap){
...
template <class ObjectType>
shared_ptr<ObjectType> ofxCMS::ObjectCollectionBase<ObjectType>::next(shared_ptr<ObjectType> instanceRef, bool wrap){
    int idx = indexOf(instanceRef.get());
...
template <class ObjectType>
shared_ptr<ObjectType> ofxCMS::ObjectCollectionBase<ObjectType>::at(unsigned int idx){
    if(idx < 0 || idx >= size()){
...
template <class ObjectType>
shared_ptr<ObjectType> ofxCMS::ObjectCollectionBase<ObjectType>::find(ObjectType* cid){
    int idx = indexOf(cid);
    if(idx == OFXCMS_INVALID_INDEX)
        return nullptr;
    return at(idx);
...
template <class ObjectType>
int ofxCMS::ObjectCollectionBase<ObjectType>::indexOf(ObjectType* cid){
    int idx=0;
...
template <class ObjectType>
void ofxCMS::ObjectCollectionBase<ObjectType>::each(IterateRefFunc func){
...
template <class ObjectType>
shared_ptr<ObjectType>  ofxCMS::ObjectCollectionBase<ObjectType>::remove(shared_ptr<ObjectType> instanceRef, bool notify){
    if(instanceRef == nullptr){
...
template <class ObjectType>
shared_ptr<ObjectType> ofxCMS::ObjectCollectionBase<ObjectType>::remove(ObjectType* cid, bool notify){
    // vector being iterated over? schedule removal operation for when iteration is done
    if(isLocked()){
...
template <class ObjectType>
shared_ptr<ObjectType> ofxCMS::ObjectCollectionBase<ObjectType>::removeByIndex(unsigned int index, bool notify){
...
template <class ObjectType>
void ofxCMS::ObjectCollectionBase<ObjectType>::lock(LockFunctor func){
    vectorLockCount++;
    func();
    vectorLockCount--;
